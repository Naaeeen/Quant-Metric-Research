...
def write_benchmark_run(
    ...
    *,
    ...
    _write_json(manifest, paths["benchmark_manifest"])
    _write_json(data_gate, paths["data_gate"])
    _write_json(acceptance, paths["acceptance"])
    result.fold_assignments.to_parquet(paths["fold_assignments"], index=False)
    tuning_trials.to_csv(paths["hyperparameter_trials"], index=False)
    result.screening_by_fold.to_csv(paths["screening_by_fold"], index=False)
    if prediction_format == "parquet":
        result.predictions.to_parquet(paths["oos_predictions"], index=False)
    else:
        result.predictions.to_csv(paths["oos_predictions"], index=False)
    result.daily_metrics.to_csv(paths["daily_metrics"], index=False)
    result.fold_metrics.to_csv(paths["fold_summary"], index=False)
    result.summary.to_csv(paths["benchmark_summary"], index=False)
